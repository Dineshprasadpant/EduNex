...
namespace EduNex.DataAccess
{
    ...
    public class ExamDal : IExamDal
    {
            ...
            using (var db = Connection)
            {
                ...
                using (var trans = db.BeginTransaction())
                {
                    try
                    {
                        ...
                        await db.ExecuteAsync(sql, ex, trans);

                        if (ex.BatchIds?.Any() == true)
                        {
                            await db.ExecuteAsync("INSERT INTO ExamBatches (ExamId, BatchId) VALUES (@ExId, @BId)",
        ...
        public async Task<IEnumerable<Exam>> FindByIdsAsync(IEnumerable<string> externalIds)
        {
            using (var db = Connection)
            {
                return await db.QueryAsync<Exam>("SELECT * FROM Exams WHERE ExternalExamId IN @Ids", new { Ids = externalIds });
            }
        }
            ...
            using (var db = Connection)
            {
                ...
                using (var multi = await db.QueryMultipleAsync(sql, new { Offset = (page - 1) * limit, Limit = limit }))
                {
                    ...
                    foreach (var ex in exams)
                        ex.PopulatedBatches = batchLinks.Where(bl => bl.ExamId == ex.Id).Select(bl => new BatchRef { Id = bl.Id, BatchName = bl.BatchName }).ToList();
        ...
        public async Task<(IEnumerable<Exam> Items, int Total)> GetByBatchAndStatusAsync(Guid batchId, string status, int page, int limit)
        {
            using (var db = Connection)
            {
                ...
                using (var multi = await db.QueryMultipleAsync(sql, new { BId = batchId, Offset = (page - 1) * limit, Limit = limit }))
                {
        ...
        public async Task<IEnumerable<Guid>> GetUserAttendedExamIdsAsync(Guid userId)
        {
            using (var db = Connection)
            {
                return await db.QueryAsync<Guid>("SELECT ExamId FROM UserExamAttempts WHERE UserId = @UId", new { UId = userId });
            }
        }
        ...
        public async Task<bool> UpdateAsync(string externalId, Exam ex)
        {
            using (var db = Connection)
            {
                ...
                return await db.ExecuteAsync(sql, new { ex.Title, ex.Description, ex.ExamName, ex.StartDateTime, ex.EndDateTime, ExtId = externalId }) > 0;
            }
        }

        public async Task<bool> DeleteAsync(Guid id)
        {
            using (var db = Connection) => await db.ExecuteAsync("DELETE FROM Exams WHERE Id = @Id", new { Id = id }) > 0;
        }
    }
}
