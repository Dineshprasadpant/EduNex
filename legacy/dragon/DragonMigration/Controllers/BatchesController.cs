...
namespace EduNex.API.Controllers
{
    ...
    public class BatchesController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromBody] Batch batch) => Ok(await _batchService.CreateBatchAsync(batch));

        [HttpGet]
        [Authorize(Roles = "admin,teacher")]
        public async Task<IActionResult> GetAll(int page = 1, int limit = 10) => Ok(await _batchService.GetAllBatchesAsync(page, limit));
        ...
        [HttpPut("{id}")]
        [Authorize(Roles = "admin,teacher")]
        public async Task<IActionResult> Update(Guid id, [FromBody] Batch batch) => Ok(await _batchService.UpdateBatchAsync(id, batch));
