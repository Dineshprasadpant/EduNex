...
namespace EduNex.DataAccess
{
    ...
    public class QuestionSheetDal : IQuestionSheetDal
    {
            ...
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                ...
                using (var trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        await conn.ExecuteAsync("INSERT INTO QuestionSheets (Id, SheetName, CreatedAt) VALUES (@Id, @SheetName, SYSUTCDATETIME())", sheet, trans);

                        foreach (var q in sheet.Questions)
                        {
                            q.Id = Guid.NewGuid();
                            await conn.ExecuteAsync("INSERT INTO Questions (Id, SheetId, QuestionText, Marks, CorrectAnswer) VALUES (@Id, @SheetId, @Question, @Marks, @CorrectAnswer)", 
                                new { q.Id, SheetId = sheet.Id, q.question, q.Marks, q.CorrectAnswer }, trans);

                            if (q.Answers?.Any() == true)
                            {
                                await conn.ExecuteAsync("INSERT INTO QuestionOptions (QuestionId, OptionText) VALUES (@QId, @Opt)",
        ...
        public async Task<bool> UpdateAsync(Guid id, QuestionSheet sheet)
        {
            ...
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                ...
                using (var trans = conn.BeginTransaction())
                {
                    try
                    {
                        await conn.ExecuteAsync("DELETE FROM QuestionSheets WHERE Id = @Id", new { Id = id }, trans);
                        sheet.Id = id;
                        await conn.ExecuteAsync("INSERT INTO QuestionSheets (Id, SheetName, CreatedAt) VALUES (@Id, @SheetName, SYSUTCDATETIME())", sheet, trans);
                        ...
                        {
                            q.Id = Guid.NewGuid();
                            await conn.ExecuteAsync("INSERT INTO Questions (Id, SheetId, QuestionText, Marks, CorrectAnswer) VALUES (@Id, @SheetId, @QuestionText, @Marks, @CorrectAnswer)", 
                                new { q.Id, SheetId = sheet.Id, q.question, q.Marks, q.CorrectAnswer }, trans);
        ...
        public async Task<bool> DeleteAsync(Guid id)
        {
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                return await conn.ExecuteAsync("DELETE FROM QuestionSheets WHERE Id = @Id", new { Id = id }) > 0;
            }
        }
        ...
        public async Task<(IEnumerable<QuestionSheet> Items, int Total)> GetAllPaginatedAsync(int page, int limit)
        {
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                ...
                using (var multi = await conn.QueryMultipleAsync(sql, new { Offset = (page - 1) * limit, Limit = limit }))
                {
                    ...
                    var items = await multi.ReadAsync<QuestionSheet>();
                    return (items, total);
                }
            }
        }
