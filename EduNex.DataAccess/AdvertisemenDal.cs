...
namespace EduNex.DataAccess
{
    ...
    public class AdvertisementDal : IAdvertisementDal
    {
        ...
        public async Task<Guid> CreateAsync(Advertisement ad)
        {
            ...
            {
                ad.Id = Guid.NewGuid();
                const string sql = "INSERT INTO Advertisements (Id, Title, Description, ImageUrl, LinkUrl, CreatedAt) VALUES (@Id, @Title, @Description, @ImageUrl, @LinkUrl, @CreatedAt)";
        ...
        public async Task<bool> UpdateAsync(Guid id, Advertisement ad)
        {
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                const string sql = "UPDATE Advertisements SET Title = @Title, Description = @Description, ImageUrl = @ImageUrl, LinkUrl = @LinkUrl WHERE Id = @Id";
